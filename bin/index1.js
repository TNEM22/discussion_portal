...
function questionTabHandler(e) {
  ...
  if (task === "resolve") {
    ...
    const maxUpvote = Number(
      count.parentNode.firstChild.querySelector(".options p").innerText
    );
    if (Number(curr_votes.innerText) > maxUpvote) {
      const parent = count.parentNode;
      count.remove();
      parent.insertBefore(count, parent.firstChild);
    }
    saveDiscussion();
    ...
    const uncle = parent.nextSibling;
    const parent_votes = Number(parent.querySelector(".options p").innerText);
    const uncle_votes = Number(uncle.querySelector(".options p").innerText);
    if (uncle_votes > parent_votes) {
      if (
        Number(
          parent.parentNode.firstChild.querySelector(".options p").innerText
        ) < uncle_votes
      ) {
        uncle.remove();
        parent.parentNode.insertBefore(uncle, parent.parentNode.firstChild);
      }
    }
    saveDiscussion();
  } else if (task === "delete") {
...
function updateTimers() {
  ...
  timers.forEach((timer) => {
    ...
    if (seconds_gone < 60) {
      ...
      timer.innerText = `Added ${Math.floor(seconds_gone / 60)} minute ago.`;
    } else {
      timer.innerText = `Added ${Math.floor(seconds_gone / 60 / 60)} hour ago.`;
    }
  });
}
